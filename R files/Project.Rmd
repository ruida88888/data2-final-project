...
In our project, we test the hypothesis that terrorism has a negative effect on housing prices. We have chosen to focus on the July 7, 2005 London bombings. We want to use an event study regression design to check the average housing prices around the bombings between June to October, and the trading volume variation before and after the bombing. A priori, we expect a decrease in average housing prices, and a decrease in trading volume after the bombing. Before we begin our analysis, we needed several data points to facilitate our analysis. Firstly, a timeline would help us filter the data into a range around July 7th, 2005. Secondly, we needed to know where the bombing happened. We analyze this throughout the greater London area, and then narrow it down to a three-district bombing area. Hence, we need to know where in greater London those four bombs were located. One of them was in the city of London, two of them were in the city of Camden, and the one left was in the city of Westminster. After the bombing, we lost 50 innocent people; 700 people were injured, and Londoners fell into a brief panic. After a thorough examination of the background, we also must ensure that our data is reliable. Hence, we have chosen to search for our data from the UK governmental data resources. Now we can begin our analysis.

My shiny app is located on: https://ruida.shinyapps.io/David_Final/
...
## Plot #1 (Static): The Average UK House Prices in 2005 vs 2010 in the three cities that were targeted in 2005 Bombings
...
## Plot #2 (Static): The Average UK House Prices 30 Days Before and After The Bombing
...
## Plot #3 (Interactive): The Average UK House During The Weeks Before and After Bombing
...
## Plot #4 (Static): Map Showing Targeted Wards
...
# 3. Text processing
Here we look at article (https://www.britannica.com/event/London-bombings-of-2005) published as a report of the London Bombings of 2005 and we quantify the most common 20 words
...
# 4. Analysis
Here we generate a linear regression model to predict the changes in UK price housing in the three wards affected by the 2005 UK bombings. Our independent variables are the ward, and the days to bombing
...
The model Adjusted R-squared is: `r prettyNum(summary(model)$adj.r.squared, digits=2)`, which means that ~20% of the UK house prices cases are explainable using this model, and the p-value is lower than alpha = 0.05 which indicates that this model is signficiant. RSE is very high indicating an extansive error with predictions. The coefficients estimate for the independent variables indicate that only 'City: CITY OF LONDON' was signficiant variable, while days to bombing was not. This concludes that the 2005 UK bombing did not have a significant impact on UK house prices.
